...
const T_NO := preload("res://models/tiles/no_bev.glb")      # free: none
const T_ONE := preload("res://models/tiles/one_bev.glb")    # free: +Z
const T_TWO := preload("res://models/tiles/two_bev.glb")    # free: +X, +Z
const T_THREE := preload("res://models/tiles/three_bev.glb")# free: +X, -Z, +Z
const T_OPP := preload("res://models/tiles/two_opp_bev.glb")# free: -Z, +Z (opposite)
const T_FULL := preload("res://models/tiles/full_bev.glb")  # free: all
...
## When scattering grass on the map surface, skip triangles whose face normal
## leans further from straight up than this (dot with world up). Keeps grass off
## the ground's vertical side walls / cliff faces. 0 = scatter on any face,
## 1 = only on perfectly flat tops.
@export_range(0.0, 1.0) var grass_surface_min_up: float = 0.4
## Optional node holding the water surface mesh(es). With follow_map_height on,
## tiles the map mesh doesn't cover (open water) would otherwise keep the base
## height and sit ABOVE a sunken map, making the cat step UP onto the water.
## Set this so those tiles take the water surface height and read lower than land.
...
## Buildings/structures (including trees) are only solid for collision purposes
## in roughly this many metres above their own base. A tree's canopy is much
## wider than its trunk, and without this the whole canopy footprint would read
## as a wall, blocking tiles beside the trunk that are perfectly walkable at cat
## height. Clipping the COLLISION MESH itself (rather than filtering it in the
## player's wall-check) keeps the check simple and reliable for ordinary walls,
## which are unaffected since they're solid well past this height anyway.
@export var structure_collision_height: float = 1.2
...
## Where the crate's goal pad sits. Leave at (-1, -1) to pick a random free tile
## (the old behaviour); set an explicit tile to author the puzzle deliberately.
@export var crate_goal_tile: Vector2i = Vector2i(-1, -1)
...
## Smaller = tighter culling (fewer blades drawn per frame) but more draw calls;
## larger = fewer draw calls but a wider off-screen margin gets drawn. Tune per
## scene: big fields (scene 6) want this small, e.g. 8.
@export var grass_chunk_size: int = 2
## Cap how many distinct blade shapes load per height tier (short/regular/tall).
## 0 = load every blade in the set (the default look). A small value (e.g. 3) cuts
## the number of MultiMeshes — and therefore draw calls — on huge fields without
## changing blade COUNT (density) at all, only shape variety. Random yaw, size and
## colour still keep the field varied.
@export var grass_blades_per_tier: int = 0
...
@export_subgroup("Density Variation")
## Vary grass density across the field instead of a flat blade count everywhere:
## grass_per_tile / grass_per_area act as the MAX density and areas thin out from
## there (down to grass_density_min) using the height signal + a noise field. Tall
## grass reads sparser, short grass denser, with noise for natural variation. Off =
## the old uniform density. Blades are dropped at scatter time, so a thinner field
## also means fewer blades drawn (a performance win) wherever it thins.
@export var grass_density_variation: bool = true
## The sparsest the field ever gets, as a fraction of the max density (1.0 = no
## thinning anywhere; 0.5 = the sparsest areas keep half their blades).
@export_range(0.05, 1.0) var grass_density_min: float = 0.5
## How strongly grass HEIGHT drives density. 1 = purely inverse to height (tall =
## sparse); 0 = height ignored, density varies by noise only. In between blends the
## two, so tall areas trend sparse but still vary blade-to-blade.
@export_range(0.0, 1.0) var grass_density_height_priority: float = 0.7
## Feature size of the density-variation noise (higher = smaller, more frequent
## dense/sparse patches).
@export var grass_density_noise_frequency: float = 0.08
...
## Start the scene faded to black and fade in on load — the "wake at dawn" transition.
## Pairs with end_day() (fade out on sleep) to make the day loop feel like sleeping.
@export var start_faded: bool = false
## Seconds for the day-transition fades (fade out on sleep, fade in on wake).
@export var day_fade_time: float = 1.2
...
## Spawn a goal pad the ball must be rolled onto. Turn OFF for a ball that just needs
## a nudge (no target), so no stray pad appears.
@export var spawn_ball_goal: bool = true
...
const _GRASS_FIELD_SCRIPT := "res://scripts/world/grass_field.gd"
...
const _CLOUD_SHADOWS_SCRIPT := "res://scripts/world/cloud_shadows.gd"
...
# Density variation: a lazily-built noise field that, blended with the height signal,
# drives how many blades survive at each point (see grass_density_at).
var _grass_density_noise: FastNoiseLite
...
# Full-screen fade for the sleep/wake day transitions (see screen_fade.gd).
...
func _ready() -> void:
	...
	# Day-transition fade LAST so it sits on top of everything. Starts black and fades
	# in when start_faded is on (the wake-at-dawn beat); end_day() fades it back out.
	_fade = ScreenFade.new()
	_fade.name = "ScreenFade"
	add_child(_fade)
	_fade.setup(start_faded)
	if start_faded:
		_fade.from_black(day_fade_time)

# --- Day loop (sleep to end the day, wake at the same dawn) ----------------

## Advance the time of day by one phase (morning -> evening -> night ...). Driven by
## story progress from a SceneFlow, not a real-time timer.
func advance_time_phase() -> void:
	if _day_night:
		_day_night.advance()

## Jump the time of day straight to a specific phase index (see DayNightCycle / the
## day_phases list). Used by a flow to set "evening" or "night" at a beat.
func set_time_phase(index: int) -> void:
	if _day_night:
		_day_night.set_phase(index)

## The phase index currently applied (0 = the first/morning phase).
func time_phase() -> int:
	return _day_night.current_phase() if _day_night else 0

## End the day: let night fall, fade to black, advance the loop counter, and reload the
## scene so the town resets and the cat wakes at the same dawn. GameState (an autoload)
## survives the reload, so a flow can tell it has looped by reading current_day.
func end_day() -> void:
	if _day_night:
		set_time_phase(_day_night.phase_count() - 1)          # last phase = night
		await get_tree().create_timer(day_transition_time + 0.2).timeout   # watch it fall
	if _fade:
		await _fade.to_black(day_fade_time)
...
func _build_buildings_collision() -> void:
	...
	for mi in meshes:
		_create_base_collision(mi as MeshInstance3D, structure_collision_height)

## Build collision for `mi`, but only from the part of its mesh within `height`
## metres of the mesh's OWN lowest vertex (found in world space, so this works
## regardless of where the asset's pivot/origin is). A plain create_trimesh_collision()
## covers the entire mesh, so a tree's canopy — much wider than its trunk — reads
## as a wall on every tile it overhangs. Dropping the tall geometry here, at the
## source, means the player's ordinary wall-check needs no special-casing: there's
## simply no collider left above the band for it to hit.
func _create_base_collision(mi: MeshInstance3D, height: float) -> void:
	if mi == null:
		return
	var mesh := mi.mesh
	if mesh == null:
		return
	var faces := mesh.get_faces()      # local-space triangle soup, 3 verts/face
	if faces.is_empty():
		return
	...
	if not is_finite(base_y):
		return
	var clip_y := base_y + height
	# Keep whole triangles that dip into the band at all (at least one vertex at
	# or below clip_y). This preserves the solid base/trunk intact while dropping
	# geometry that's ENTIRELY above the band (a canopy, a roof cap).
	...
	while i + 2 < faces.size():
		...
		var lo := minf((xform * a).y, minf((xform * b).y, (xform * c).y))
		if lo <= clip_y:
			# Bake mi's transform in so `kept` really is in WORLD space, matching
			# the identity body transform set below. (Appending the raw local
			# verts here placed every collider at the mesh's local origin instead
			# of where the structure actually sits — the cat clipped through
			# everything because the collider was offset from the visible mesh.)
			kept.append(xform * a)
			kept.append(xform * b)
			kept.append(xform * c)
		i += 3
	...
	body.add_child(cs)
	mi.add_child(body)
	# `kept` is already in WORLD space (we baked mi's transform into it above), so
	# the body itself must sit at the identity transform, not inherit mi's.
...
func _scan_map_surface() -> void:
	...
	for s in mi.mesh.get_surface_count():
		...
		for ti in tri_n:
			...
			# but only on up-facing faces so blades don't sprout from the vertical
			# side walls / cliff edges of the ground.
			if is_grass:
				var fn := (b - a).cross(c - a)
				if fn.length() > 0.0000001 and absf(fn.normalized().y) >= grass_surface_min_up:
	...
	# Tiles the map mesh never covers (open water) still hold the base height,
	# which sits ABOVE a sunken map and makes the cat step UP onto the water.
	# Drop them to the water surface so water reads lower than the land.
	if follow_map_height:
		var water_elev := _water_surface_elevation()
		if not is_nan(water_elev):
			for tx in grid_size:
				for tz in grid_size:
					if not got.has(Vector2i(tx, tz)):
						_heights[tx][tz] = water_elev

## World-space top Y of the water mesh(es) under water_path, converted to the
## tile elevation convention (surface Y minus the 0.5 standing offset). Returns
## NAN when no water_path / mesh is set.
func _water_surface_elevation() -> float:
	var wnode := get_node_or_null(water_path)
	if wnode == null:
		return NAN
	var top := -INF
	for m in _all_mesh_instances(wnode):
		if m.mesh == null:
			continue
...
func _spawn_water() -> void:
	if water_tiles.is_empty():
		return
	var scene: PackedScene = load("res://models/props/water.glb")
	if scene == null:
...
## Keep fraction (0..1) for a blade at world XZ — how likely a candidate blade is to
## survive when density variation is on. 1.0 = keep everything (the max density set by
## grass_per_tile / grass_per_area); lower thins the field out. Short grass trends
## toward 1.0 (denser) and tall grass toward grass_density_min (sparser), blended with
## a noise field so the thinning varies naturally rather than tracking height exactly.
## Returns 1.0 when the feature is off, so scatter keeps every blade (the old look).
func grass_density_at(wx: float, wz: float) -> float:
	if not grass_density_variation:
		return 1.0
	# Short grass -> 1, tall grass -> 0, so the tallest areas thin out the most.
	var height_term := 1.0 - grass_height_at(wx, wz)
	if _grass_density_noise == null:
		_grass_density_noise = FastNoiseLite.new()
		_grass_density_noise.seed = terrain_seed + 555
		_grass_density_noise.frequency = grass_density_noise_frequency
		_grass_density_noise.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH
	var noise_term := _grass_density_noise.get_noise_2d(wx, wz) * 0.5 + 0.5
	# Blend height-driven and noise-driven thinning, then map onto [min .. 1].
	var w := clampf(lerpf(noise_term, height_term, grass_density_height_priority), 0.0, 1.0)
	return lerpf(clampf(grass_density_min, 0.0, 1.0), 1.0, w)
...
func _spawn_blocks() -> void:
	if block_tiles.is_empty():
		return
	_crate_scene = load("res://models/props/crate.glb")
...
func _on_click(event: InputEvent, catcher: Control) -> void:
	...
	if not _player.has_method("set_path"):
		return
	# Shift+click sprints to the clicked tile instead of the normal walk pace.
	var sprint_to: bool = event is InputEventMouseButton and event.shift_pressed
	...
	if delta != Vector2i.ZERO and (delta.x == 0 or delta.y == 0):
		...
		if _is_pushable_tile(start + dir):
			_player.set_path(_straight_line(start, dir, tile), sprint_to)
			return

	# Otherwise pathfind around obstacles.
	var path := find_path(start, tile)
	if not path.is_empty():
		_player.set_path(path, sprint_to)
...
func _spawn_goal() -> void:
	...
	if not block_tiles.is_empty():
		_add_goal("res://models/tiles/goal_tile.glb", "res://models/tiles/goal_tile_triggered.glb", "crate", crate_goal_tile)
	if not ball_tiles.is_empty() and spawn_ball_goal:
		_add_goal("res://models/tiles/goal_tile_b.glb", "res://models/tiles/goal_tile_b_triggered.glb", "ball")

func _add_goal(unt_path: String, tri_path: String, by: String, tile_override := Vector2i(-1, -1)) -> void:
	...
	var use_tile := tile_override if _tile_in_bounds(tile_override) else _pick_goal_tile()
	var goal := {
		"tile": use_tile, "untriggered": unt, "triggered": tri,
...
## Is a tile inside the grid? (Used to accept an explicit goal-tile override.)
func _tile_in_bounds(t: Vector2i) -> bool:
	return t.x >= 0 and t.x < grid_size and t.y >= 0 and t.y < grid_size

## Is the crate's goal currently triggered? A live predicate for a SceneFlow to gate on.
func crate_goal_won() -> bool:
	for g in _goals:
		if g["by"] == "crate":
			return g["won"]
	return false
...
func _spawn_balls() -> void:
	if ball_tiles.is_empty():
		return
	_ball_scene = load("res://models/props/ball.glb")
	if _ball_scene == null:
