...
## Number of phases in the cycle, and the index currently applied. Let a flow pick a
## specific time of day (e.g. jump straight to Evening) rather than stepping through.
...
## Set the time of day to a specific phase index (eased by default, instant if
## animate = false). Out-of-range indices are clamped.
func set_phase(index: int, animate := true) -> void:
	if _phases.is_empty():
		return
	_index = clampi(index, 0, _phases.size() - 1)
	_apply_phase(_phases[_index], animate)
