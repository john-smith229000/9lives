...
##
## Sprint: tapping Shift ("sprint" action) TOGGLES sprint mode on/off, speeding up
## both grid-step and free-mode movement until tapped again. Shift+click instead
## sprints just for that one click-to-move path, then reverts automatically.
...
## --- Sprint (tap Shift to toggle, or Shift+click) ------------------------
## Grid-glide top speed while sprinting is move_speed * this.
@export var sprint_speed_mult: float = 1.8
## Free-mode (interior) speed while sprinting is free_speed * this.
@export var sprint_free_mult: float = 1.6
## Walk animation plays this much faster while sprinting (feet keep pace).
@export var sprint_anim_mult: float = 1.35
...
## Max height (m) of a crate the cat will STEP ONTO by walking (crossing between
## equal-height crates). A crate whose top sits higher than this above the cat is
## PUSHED instead of mounted. Kept separate from max_walk_step so a big terrain step
## height (for slopes) doesn't accidentally turn a pushable crate into a stair.
@export var max_mount_step: float = 0.4
...
var _path_sprint := false     # true while the CURRENT path was a shift+click sprint-to

# --- Sprint ---
var _sprint_toggle := false   # persistent on/off state from tapping the sprint key
...
func _process(delta: float) -> void:
	# Tap Shift to toggle sprint on/off. Checked up front so it works no matter
	# what mode/state the cat is in (frozen only during a conversation).
	if Input.is_action_just_pressed("sprint") and not Dialogue.is_active():
	...
	if not _moving and dir != Vector2i.ZERO:
		...
		if not _begin_segment(dir) and not _jumping:
			...
			if dir != _facing:
				_orient(dir, 0.0)
			_clear_path()        # path blocked — abandon it
	if _moving:
		_advance(delta, dir)
	else:
...
## True while sprint is in effect: either the toggle is on (Shift tapped), or the
## cat is following a shift+click sprint-to path.
func is_sprinting() -> bool:
	return _sprint_toggle or _path_sprint
...
## `sprint` marks this path as a shift+click sprint-to: the cat runs at sprint
## speed for this path only, then reverts (independent of the toggle state).
func set_path(path: Array, sprint: bool = false) -> void:
	_path = path.duplicate()
	_path_sprint = sprint

## Clear the click-to-move path and any sprint-to-location flag riding on it.
...
func _auto_dir() -> Vector2i:
	...
	while not _path.is_empty() and _path[0] == _grid:
		_path.remove_at(0)
	if _path.is_empty():
		_path_sprint = false   # path finished — sprint-to-location reverts
...
func _free_move(delta: float) -> void:
	...
	var speed := free_speed * (sprint_free_mult if is_sprinting() else 1.0)
...
func _update_anim_speed() -> void:
	...
	if _anim:
		var want := walk_anim_speed
		if _pushing:
			want *= push_speed_scale
		elif _moving and is_sprinting():
			want *= sprint_anim_mult
...
func _begin_segment(dir: Vector2i) -> bool:
	...
	var step_onto_crate := _tile_has_block(t) and not _tile_has_water(t) and climb <= max_mount_step
...
func _advance(delta: float, _dir: Vector2i) -> void:
	...
	# Pushing a block slows everything down; sprinting speeds a free (non-push) glide
	# up instead (no sprinting while pushing — the crate keeps its own pace).
	var top_speed := move_speed
	if not _push.is_empty():
		top_speed *= push_speed_scale
	elif is_sprinting():
		top_speed *= sprint_speed_mult
...
func _try_jump() -> void:
	_clear_path()                       # a jump cancels click-to-move
