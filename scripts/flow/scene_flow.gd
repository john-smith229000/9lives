...
## Pulsing outline. `scale` is the hull thickness (1.06 = 6% larger); pass a value
## to override the default prop thickness — e.g. Timing.char_outline_scale so an NPC's
## scripted highlight matches its (thinner) hover outline.
func highlight(node: Node3D, scale := -1.0) -> void:
	if node:
		var sc := scale if scale > 0.0 else Timing.outline_scale
		Outline.add(node, Timing.outline_color, sc,
...
##   highlight_scale  : outline thickness override (e.g. Timing.char_outline_scale)
...
func beat(cfg: Dictionary) -> void:
	...
	if node:
		highlight(node, cfg.get("highlight_scale", -1.0))
...
func spawn_arrow(over: Node3D, model_path := "res://models/props/arrow.glb") -> Node3D:
	if over == null:
		return null
