...
**Performance / culling.** Blades are bucketed into square **chunks** (`grass_chunk_size` tiles across), each its own MultiMesh, so off-screen chunks are frustum-culled. On a big field (scene 6 is ~100×60 m, ~1M blades) the chunk size is the main FPS lever: it does *not* change how many blades exist (density = `grass_per_area`), only how many are **drawn per frame**. Coarse chunks make the cull draw a wide off-screen margin; halving the chunk size (scene 6 uses `grass_chunk_size = 8`) roughly halves drawn blades. Because each height tier × blade-shape is a separate MultiMesh, smaller chunks mean more draw calls — `grass_blades_per_tier` (scene 6 uses `3`) caps shapes per tier to keep draw calls flat, again without touching density (random yaw/size/colour keep the field varied). Rule of thumb: GPU-bound (low FPS) → smaller `grass_chunk_size`; CPU/draw-call-bound → larger.
